...
class MatchScheduleDetailActivity : AppCompatActivity(), MatchScheduleDetailView {
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        item?.itemId?.let {
            when (it) {
                ...
                R.id.favorite -> {
                    if (!isFavorite) {
                        mPresenter.insertMatchDataToFavorite(this, event.idEvent, event.idHomeTeam, event.idAwayTeam)
                        isFavorite = !isFavorite
                        Snackbar.make(main_layout, R.string.title_add, Snackbar.LENGTH_SHORT).show()
                    } else {
                        mPresenter.deleteMatchDataToFavorite(this, event.idEvent)
                        isFavorite = !isFavorite
    ...
    override fun onTeamDetailDataReceived(teamData: TeamsDto, isHomeTeam: Boolean) {
        teamData.let { detail ->
    ...
    private fun onAttachView() {
        mPresenter.requestTeamDetailData(event.idHomeTeam, true)
        mPresenter.requestTeamDetailData(event.idAwayTeam, false)
        mPresenter.selectMatchDataFromFavorite(this, event.idEvent)
    }
